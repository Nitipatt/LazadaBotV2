// In checkout page, Go to network tab and select offline mode 
// Here is the first function that is called when clicked place order button
// Find function Ki(a, b, c, d) in this file
(function() {
    'use strict';
    (function(c, x) {
...
(function() {
    ...
    (function(M, ha) {
        ...
        // while loop to find placeOrderBtn
        const clearIntervalById = (id)=> clearInterval(id);
        if(window.sessionStorage.getItem("bot_listenClickRegister")){
            const intervalId = setInterval(()=>{
                const btn = document.querySelector('.i-button-content')?.parentNode;
                console.log('Finding btn');
                if(btn){
                    console.log('Found btn', new Date().getTime());
                    if(!btn.disabled){
                        //btn.click();
                        console.log('clicked');
                        clearIntervalById(intervalId);
                    }else {
                        const trigger = new Date('2024-05-05 00:00:00').getTime();
        ...
        function Ki(a, b, c, d) {
            ...
            (function(){
                if (
                    ...
                    "https://www.lazada.co.th/wow/gcp/th/trade/shipping"
                ) {
                    const placeOrderBtnText = document.querySelector('.i-button-content');
                    const placeOrderBtn = placeOrderBtnText?.parentNode
                    if(placeOrderBtn){
                        if(!window.sessionStorage.getItem("bot_listenClickRegister")){
                            placeOrderBtn.addEventListener("click", ()=>{
                                console.log('Place order clicked');
                                placeOrderBtnText.innerText = "Auto Place Order";

                                const urlParams = new URLSearchParams(window.location.search);
                                const buyParams = urlParams.get('buyParams');
                                const channel = new BroadcastChannel(buyParams); //Broadcast channel
                                channel.onmessage = ({data}) => {
                                      switch (data.type) {
                                        case 'refresh':
                                          if(document.querySelector('.i-button-content')?.parentNode?.disabled){
                                              console.log('refresh');
                                              window.location.reload();
                                          }
                                          break;
                            ...
                            window.sessionStorage.setItem("bot_listenClickRegister", true);
                        }
                        if(placeOrderBtn.disabled){
                            placeOrderBtnText.innerText = "Click to Auto";
                        }
                    }
                }
            })();
            try {
                const storeTrust = window.storeTrust ?? {};
                if(d.isTrusted){
                    storeTrust[a] = d;
                    window.storeTrust = storeTrust
                    console.log('storeTrust', storeTrust)
                }
                const mimicD = storeTrust?.[a] ?? d
                console.log("Ki -> a",a)
                console.log("Ki -> d",mimicD)
                fg(e, a, b, c, mimicD);
            } finally {
                (Xa = f) || Wd()
            }
